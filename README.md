...
A maintainer gets a bug report. Before they spend time on it, Understudy tries to run it.

For each inbound issue: it reads the issue text and the checked-out repo (no `git`, that's
blocked — see Safety below), writes a Python script meant to reproduce the reported bug,
executes it in a locked-down container, and if the script itself is broken (not the bug —
the script), reads the error back and tries again, up to three attempts. It reports one of
four verdicts: `candidate_reproduction`, `not_reproduced`, `insufficient_info`, or `error`.

It's allowed to act on that on its own — post a comment, request the one missing detail. But
interrupting a human costs from a daily budget, enforced in code, not by asking politely.
Most triaged issues should never need to.

## Why this is hard to fake, and how we checked

A script that exits non-zero doesn't mean the bug is real — an `ImportError` or a wrong API
call also exits non-zero. So the only thing this project trusts is the **gold-patch
...
We ran both checks side by side on the same 16 real SWE-bench Verified instances:
...
bug and kept failing after the real fix was applied — proof the failure had nothing to do
with the patch. One inverted the bug's own example, mistaking the buggy output shown in the
issue for the correct one. This taxonomy, and every underlying run, is in `results.json` and
on Screen C of the live demo.

19% is still meaningfully above the published zero-shot floor (GPT-4, issue text alone:
3.6%), on a model that costs nothing to run.

## The model, and why it isn't what the plan originally said

Inference is local: **`qwen2.5-coder:7b` via Ollama**, not a cloud model. AWS Bedrock access
on this AWS account returns `ValidationException: Operation not allowed` — confirmed to
persist across a full Free-to-Paid plan upgrade, and it's the second project where this
exact failure has shown up. This submission has a hard $0 budget, so the fallback isn't a
paid API — it's local inference. The honest cost of that: a 7B local model will underperform
a frontier model on this task. We didn't assume that was fine; we measured it (see the table
above), and it's the finding this project leads with rather than hides. Full decision
history is in `COMPETITION.md`.

## Architecture

See `ARCHITECTURE.md` for the full diagram and trust-boundary writeup. Short version:
...
AgentCore + EventBridge deployment. Confirmed blocked at the account level (2026-09-12):
attaching a working `bedrock-agentcore:*` IAM policy to the user still gets
`AccessDeniedException` on every call, the same pattern as the Bedrock `ValidationException`
— an account-wide restriction above the IAM-user level, not something fixable through
permissions. The SWE-bench eval also hasn't scaled past 16 instances. Both tracked in
`COMPETITION.md`.
